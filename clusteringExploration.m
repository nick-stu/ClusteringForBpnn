function [ mdl ] = clusteringExploration( data ,isPlot)
% mdl.data: 聚类的训练样本
...
% 阈值判断为加入后当前类的平均距离
    outLabel=...
        [ones(1,30)*1 ones(1,30)*2 ones(1,30)*3 ones(1,30)*4 ones(1,30)*5];
    mdl.data = data;
    num = size(data, 1);
    label = zeros(1, num);
...
%         DMat=[DMat D];
        ...
        fprintf('%d -- %.2f -- %.2f\n',outLabel( p_label(min_i) ),avg,threshold);
        dis(i) = avg;
...
function[distance]=distanceChange(sample,traindata)
    [distance,~]=classStatus([sample;traindata]);
end