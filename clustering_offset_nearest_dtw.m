function [ mdl ] = clustering_offset_nearest_dtw( data ,isPlot)
% mdl.data: 聚类的训练样本
...
%           d = nearestPoint(data(j, :), mdl.data(cursor,:));
        ...
        [avg,~]=classStatus_dtw([mdl.data(cursor,:);data(min_i, :)]);
...
function[distance]=distanceChange(sample,traindata)
    [distance,~]=classStatus_dtw(sample,traindata);
end

function[dis]=nearestPoint(sample,classData)
    dis=pdistDtw(sample,classData);
    dis = min(dis(:));
end